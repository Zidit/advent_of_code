...
fn order_pages(rules: std.AutoHashMap(u32, std.ArrayList(u32)), page_list: []u32) []const u32 {
    while (!is_right_order(rules, page_list)) {
        for (page_list, 0..) |page, pos| {
            if (rules.get(page)) |later_pages| {
                for (later_pages.items) |later_page| {
                    const later_pos = std.mem.indexOfScalar(u32, page_list, later_page);
                    if (later_pos != null and later_pos.? < pos) {
                        const tmp = page_list[later_pos.?];
                        page_list[later_pos.?] = page_list[pos];
                        page_list[pos] = tmp;
                    }
                }
            }
...
pub fn main() !void {
    ...
    for (page_list) |pages| {
        if (!is_right_order(rules, pages)) {
            const ord_pages = order_pages(rules, pages);

            const mid_val = ord_pages[ord_pages.len / 2];
            total += mid_val;
        }
    }
