...
fn test_safe(reports: []const i8) !bool {
    var dir: i8 = 0;
    var prev: i8 = 0;
    for (reports) |v| {
        if (prev == 0) {
...
fn is_safe(reports: []const i8) !bool {
    for (0..reports.len) |i| {
        var sub_reports = std.BoundedArray(i8, 10){};
        try sub_reports.appendSlice(reports);
        _ = sub_reports.orderedRemove(i);

        if (try test_safe(sub_reports.slice()))
            return true;
    }
    return false;
}
