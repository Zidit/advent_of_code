...
fn find_path(map: Map, start_pos: Vec2, end_pos: Vec2, allocator: std.mem.Allocator) !std.ArrayList(Tile) {
    ...
    while (queue.count() > 0) {
        ...
        if (current.key.equal(end_pos)) {
            ...
            try path.append(tile);
            std.mem.reverse(Tile, path.items);
            return path;
        }
...
pub fn main() !void {
    ...
    for (path.items, 1..) |start_tile, i| {
        const start_pos = start_tile.pos;
        const start_cost: i32 = @intCast(start_tile.cost);

        for (path.items[i..]) |end_tile| {
            const end_pos = end_tile.pos;
            const end_cost: i32 = @intCast(end_tile.cost);

            const dist: i32 = @intCast(@abs(end_pos.x - start_pos.x) + @abs(end_pos.y - start_pos.y));
            if (dist > 20)
                continue;

            const time_saved = end_cost - (start_cost + dist);
            if (time_saved >= 100)
                short_cuts += 1;
        }
    }
