...
pub fn main() !void {
    ...
    const size = 71;
    const map: Map = try map_init(size + 2, size + 2, '.', allocator);
    defer allocator.free(map.data);

    for (0..map.h) |y| {
        map_set(map, Vec2.init(0, y), '#');
        map_set(map, Vec2.init(map.w - 1, y), '#');
    }

    for (0..map.w) |x| {
        map_set(map, Vec2.init(x, 0), '#');
        map_set(map, Vec2.init(x, map.h - 1), '#');
    }

    var max_lines: usize = 1024;
    var lines = std.mem.splitSequence(u8, input, "\n");
    while (lines.next()) |line| {
        const line_trimmed = std.mem.trim(u8, line, " \n\r");
    ...
    const total = try path(map, .{ .x = 1, .y = 1 }, .{ .x = size, .y = size }, allocator);
