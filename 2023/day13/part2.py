...
def test_reflection(pattern:list[list[str]], smudges:int=0):

    for i in range(0, len(pattern) - 1):
        if test_reflection_line(pattern, i,smudges):
            return i + 1

    return 0


def test_reflection_line(pattern:list[list[str]], line:int, smudges:int=0):
    #print("Test:")
...
for pattern in patterns:
    ...
    total += test_reflection(pattern, 1) * 100
    total += test_reflection(t_pattern, 1)
